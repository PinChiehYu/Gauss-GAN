...
class HashEmbedder(nn.Module):
    def __init__(self, bounding_box, grid_resolutions=[32, 64, 128], n_features_per_level=2):
        super(HashEmbedder, self).__init__()
        self.bounding_box = bounding_box
        self.grid_resolutions = grid_resolutions
        self.n_levels = len(grid_resolutions)
        self.n_features_per_level = n_features_per_level
        self.out_dim = self.n_levels * self.n_features_per_level

        self.embeddings = nn.ModuleList([nn.Embedding(2**i, self.n_features_per_level) for i in range(self.n_levels)])
    ...
    def forward(self, x):
        ...
        for i in range(self.n_levels):
            voxel_min_vertex, voxel_max_vertex, hashed_voxel_indices = get_voxel_vertices(\
                                                x, self.bounding_box, \
                                                self.grid_resolutions[i], self.log2_hashmap_size)
