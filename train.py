...
def train(rank, world_size, opt):
    ...
    SIREN = getattr(gaussian, metadata['model']) if metadata['model'] == "SPATIALGAUSSIANBASELINE" else getattr(siren, metadata['model'])
    ...
    for _ in range (opt.n_epochs):
        ...
        for i, (imgs, _) in enumerate(dataloader):
            ...
            if opt.eval_freq > 0 and (discriminator.step + 1) % opt.eval_freq == 0:
                ...
                fid_evaluation.setup_evaluation(metadata['dataset'], generated_dir, metadata['dataset_path'], target_size=128)
                ema.store(generator.parameters())
                ema.copy_to(generator.parameters())
                generator.eval()
